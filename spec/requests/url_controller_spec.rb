  ...
  describe "GET slug" do
    context "with existing slug" do
      ...
      it "track short_url event" do
        expect do
          get "/#{short_url.slug}"
        end.to change(ShortUrlEvent, :count).by(1)

        expect(response).to redirect_to(target_url.external_url)
        expect(short_url.short_url_events.size).to eq 1
      end
      ...
      it "track short_url event on different slug" do
        ...
        expect(response).to redirect_to(target_url.external_url)
        expect(another_short_url.short_url_events.size).to eq 1
      end
      ...
      it "track multiple short_url events under same slug" do
        expect do
          ...
          expect(response).to redirect_to(target_url.external_url)
        end.to change(ShortUrlEvent, :count).by(5)

        expect(short_url.short_url_events.size).to eq 5
      end
    end
